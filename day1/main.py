...
async def search(author: str=None, category: str = None):
    ...
    if author:
        books_to_return = [book for book in books_to_return if book.get('author').casefold() == author.casefold()]
    if category:
        books_to_return= [book for book in books_to_return if book.get('category').casefold() == category.casefold()]
